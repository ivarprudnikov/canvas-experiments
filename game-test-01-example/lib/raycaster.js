...
export function RayCaster(canvas, w, h, z, State) {
    ...
    this.update = function () {
        if (!this.idle) {
            this.blank(this.RES.w, this.RES.h, this.RES.hh, this.state.level.colors.sky, this.state.level.colors.ground);
            this.cast();
        }
        this.processInput();
    }
    ...
    this.loadMap = function (m, x, y) {
        var parseOk = this.state.level.parseMap(m, x, y);
        if (parseOk) {
            this.buildPalette();
            this.camera.position._x = this.state.level.spawnPoint._x;
            this.camera.position._y = this.state.level.spawnPoint._y;
    ...
    this.cast = function () {
        var hit_latitude = {_x: 0, _y: 0, type: this.state.level.CELLTYPE_OPEN};
        var hit_longitude = {_x: 0, _y: 0, type: this.state.level.CELLTYPE_OPEN};
        var distance = {_x: 0, _y: 0};
        var step = {_x: 0, _y: 0};
        var mapScale = this.RES.h / this.state.level.dimension._y;
        ...
        for (var currentSliver = 0; currentSliver < this.RES.w; currentSliver += this.sliverWidth) {
            ...
            if (distance._x < distance._y) {
                ...
                wallScale = this.state.level.CELL_SIZE / distance._x;
                ...
                brightness = 1 - Math.min(1, distance._x / this.state.level.viewExtent);
                ...
                wallScale = this.state.level.CELL_SIZE / distance._y;
                rlu = rayDirection - this.camera.direction;
                if (rlu < 0) {
                    rlu += this.TABLE_ENTRIES;
                } else if (rlu >= this.TABLE_ENTRIES) {
                ...
                brightness = 1 - Math.min(1, distance._y / this.state.level.viewExtent);
    ...
    this.cast_north = function (hit, distance, step, ray) {
        ...
        var cellBoundY = this.camera.position._y >> this.state.level.CELL_SIZE_SHIFT;
        hit._y = (cellBoundY + 1) << this.state.level.CELL_SIZE_SHIFT;
        hit._x = this.camera.position._x + ((hit._y - this.camera.position._y) * this.TABLE_INV_TAN[ray]);
        step._x = this.state.level.CELL_SIZE * this.TABLE_INV_TAN[ray];
        step._y = this.state.level.CELL_SIZE;

        var casting = true;
        while (casting) {
            // is current hit point out of bounds?
            if ((hit._x < 0) || (hit._x >= this.state.level.dimension._x)) {
                distance._x = this.INFINITY;
                casting = false;
            } else {
                // is there a wall at the cell boundary north of the hitpoint?
                // walltype = this.state.level.map[row][col];
                hit.type = this.state.level.map[((hit._y + this.state.level.CELL_HALF) >> this.state.level.CELL_SIZE_SHIFT)][(hit._x >> this.state.level.CELL_SIZE_SHIFT)];
                if (hit.type != this.state.level.CELLTYPE_OPEN) {
    ...
    this.cast_south = function (hit, distance, step, ray) {
        ...
        var cellBoundY = this.camera.position._y >> this.state.level.CELL_SIZE_SHIFT;
        hit._y = cellBoundY << this.state.level.CELL_SIZE_SHIFT;
        hit._x = this.camera.position._x + ((hit._y - this.camera.position._y) * this.TABLE_INV_TAN[ray]);
        step._x = -this.state.level.CELL_SIZE * this.TABLE_INV_TAN[ray];
        step._y = -this.state.level.CELL_SIZE;
    ...
    this.cast_west = function (hit, distance, step, ray) {
        ...
        var cellBoundX = this.camera.position._x >> this.state.level.CELL_SIZE_SHIFT;
        hit._x = cellBoundX << this.state.level.CELL_SIZE_SHIFT;
        hit._y = this.camera.position._y + ((hit._x - this.camera.position._x) * this.TABLE_TAN[ray]);
        step._x = -this.state.level.CELL_SIZE;
        step._y = -this.state.level.CELL_SIZE * this.TABLE_TAN[ray];

        var casting = true;
        while (casting) {
            // is current hit point out of bounds?
            if ((hit._y < 0) || (hit._y >= this.state.level.dimension._y)) {
                distance._y = this.INFINITY;
                casting = false;
            } else {
                // is there a wall at the cell boundary west of the hitpoint?
                // walltype = this.state.level.map[row][col];
                hit.type = this.state.level.map[(hit._y >> this.state.level.CELL_SIZE_SHIFT)][((hit._x - this.state.level.CELL_HALF) >> this.state.level.CELL_SIZE_SHIFT)];
                if (hit.type != this.state.level.CELLTYPE_OPEN) {
    ...
    this.cast_east = function (hit, distance, step, ray) {
        ...
        var cellBoundX = this.camera.position._x >> this.state.level.CELL_SIZE_SHIFT;
        hit._x = (cellBoundX + 1) << this.state.level.CELL_SIZE_SHIFT;
        hit._y = this.camera.position._y + ((hit._x - this.camera.position._x) * this.TABLE_TAN[ray]);
        step._x = this.state.level.CELL_SIZE;
        step._y = this.state.level.CELL_SIZE * this.TABLE_TAN[ray];
    ...
    this.processInput = function () {
        ...
        console.log("processing input", this.state.input.left)
        ...
        if (this.state.input.left) {
            ...
            this.idle = false;
            trace('turning left');
            this.camera.direction -= this.state.player.speed.turn;
        ...
        if (this.state.input.up) {
            ...
            var newX = this.camera.position._x + this.state.player.speed.forward / this.TABLE_INV_COS[this.camera.direction];
            var newY = this.camera.position._y + this.state.player.speed.forward / this.TABLE_INV_SIN[this.camera.direction];
            var row = newY >> this.state.level.CELL_SIZE_SHIFT;
            var col = newX >> this.state.level.CELL_SIZE_SHIFT;
            if (this.state.level.map[row][col] == this.state.level.CELLTYPE_OPEN) {
        ...
        if (this.state.input.down) {
            ...
            var newX = this.camera.position._x - this.state.player.speed.backward / this.TABLE_INV_COS[this.camera.direction];
            var newY = this.camera.position._y - this.state.player.speed.backward / this.TABLE_INV_SIN[this.camera.direction];
            var row = newY >> this.state.level.CELL_SIZE_SHIFT;
            var col = newX >> this.state.level.CELL_SIZE_SHIFT;
            if (this.state.level.map[row][col] == this.state.level.CELLTYPE_OPEN) {
                this.camera.position._x = newX;
    ...
    this.buildPalette = function () {
        ...
        for (var i = 0; i < this.state.level.walltypes.length; i++) {
            // grab wallcolor near and wallcolor far
            var wcn = this.state.level.colors.wallsNear[i];
            var wcf = this.state.level.colors.wallsFar[i];
